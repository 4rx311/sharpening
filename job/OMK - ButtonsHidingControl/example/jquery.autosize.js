    ...
    if ($.isFunction(test[oninput]) || onpropertychange in test) {
        $.fn.autosize = function(className) {
            return this.each(function() {
                ...
                var minHeight = ta.style.height;
                if (minHeight != null && minHeight.toString().indexOf('px') != -1)
                    minHeight = minHeight.toString().split('px')[0];

                // Using mainly bare JS in this function because it is going
                // to fire very often while typing, and needs to very efficient.
                function adjust(externalCall) {
                    var height, overflow;
                    // the active flag keeps IE from tripping all over itself.  Otherwise
                    // actions in the adjust function will cause IE to call adjust again.
                    if (!active || externalCall) {
                        active = true;

                        if (ta.tagName == null || ta.parentNode == null)
                            return;
                        ...
                        if (window.ListForm != null) {
                            if (this.PreviosHeight == null)
                                this.PreviosHeight = this.offsetHeight;
                            else {
                                if (this.PreviosHeight == this.offsetHeight)
                                    return;
                                else
                                    this.PreviosHeight = this.offsetHeight;
                            }
                ...
                this.Adjust = function() { adjust(true); };
